...
- MeshCore: a Heltec V4 (USB companion firmware v1.15.0) on `/dev/serial/by-id/usb-Espressif_Systems_heltec_wifi_lora_32_v4_*` (`/dev/ttyACM0`), enabled in the core's settings. War-driving isn't started, so it only transmits when asked.
